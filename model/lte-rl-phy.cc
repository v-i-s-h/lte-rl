...
#include "lte-rl-phy.h"
#include <ns3/lte-net-device.h>

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("LteRlPhy");

NS_OBJECT_ENSURE_REGISTERED (LteRlPhy);
...
LteRlPhy::LteRlPhy (Ptr<LteRlSpectrumPhy> dlPhy, Ptr<LteRlSpectrumPhy> ulPhy)
...
TypeId
LteRlPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteRlPhy")
    .SetParent<Object> ()
    .SetGroupName("Lte")
  ;
  return tid;
...
void
LteRlPhy::SetDevice (Ptr<LteNetDevice> d)
{
...
LteRlPhy::GetDownlinkSpectrumPhy ()
{
  return m_downlinkSpectrumPhy;
}

Ptr<LteRlSpectrumPhy> 
LteRlPhy::GetUplinkSpectrumPhy ()
{
...
void
LteRlPhy::SetDownlinkChannel (Ptr<SpectrumChannel> c)
{
...
void
LteRlPhy::SetUplinkChannel (Ptr<SpectrumChannel> c)
{
  NS_LOG_FUNCTION (this << c);
...
uint16_t
LteRlPhy::GetSrsPeriodicity (uint16_t srcCi) const
{
...
uint16_t
LteRlPhy::GetSrsSubframeOffset (uint16_t srcCi) const
{
  // from 3GPP TS 36.213 table 8.2-1 UE Specific SRS Periodicity
...
uint8_t
LteRlPhy::GetRbgSize (void) const
{
  return m_rbgSize;
}

void
LteRlPhy::SetMacPdu (Ptr<Packet> p)
{
  m_packetBurstQueue.at (m_packetBurstQueue.size () - 1)->AddPacket (p);
}

Ptr<PacketBurst>
LteRlPhy::GetPacketBurst (void)
{
...
void
LteRlPhy::SetControlMessages (Ptr<LteControlMessage> m)
{
...
LteRlPhy::GetControlMessages (void)
{
  NS_LOG_FUNCTION (this);
...
void
LteRlPhy::DoSetCellId (uint16_t cellId)
{
  m_cellId = cellId;
