...
#ifndef LTE_RL_UE_NET_DEVICE_H
#define LTE_RL_UE_NET_DEVICE_H
 ...
 */
class LteRlUeNetDevice : public LteNetDevice
{

public:
  static TypeId GetTypeId (void);

  LteRlUeNetDevice (void);
  virtual ~LteRlUeNetDevice (void);
  ...
  Ptr<LteRlUePhy> GetPhy (void) const;
   ...
   * \param enb
   */
  void SetTargetEnb (Ptr<LteRlEnbNetDevice> enb);

  /**
   * \brief Get the targer eNB where the UE is registered
   * \return the pointer to the enb
   */
  Ptr<LteRlEnbNetDevice> GetTargetEnb (void);
...
private:
  ...
  Ptr<LteRlEnbNetDevice> m_targetEnb;

  Ptr<LteUeMac> m_mac;
  Ptr<LteRlUePhy> m_phy;
  Ptr<LteUeRrc> m_rrc;
  Ptr<EpcUeNas> m_nas;
